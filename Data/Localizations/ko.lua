...
L["Add item rules to automatically roll need, greed or pass when a Gargul roll is announced (does not work with group loot rolls). Rules are stored per item ID. When you win an item, its rule is automatically set to pass. Shift-click items or drag them into the box below."] = "Gargul 롤이 발표될 때 자동으로 니드, 그리드 또는 패스하도록 아이템 규칙을 추가합니다 (그룹 전리품 롤에서는 작동하지 않음). 규칙은 아이템 ID별로 저장됩니다. 아이템을 획득하면 해당 규칙이 자동으로 패스로 설정됩니다. Shift+클릭으로 아이템을 추가하거나 아래 상자로 드래그하세요.";
L["Add rule"] = "규칙 추가";
...
L["Item link or ID"] = "아이템 링크 또는 ID";
